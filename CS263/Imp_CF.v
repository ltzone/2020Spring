...
(* INSTRUCTORS: Here is a list of changes that I've made from the original version:
1. ANum takes a Z instead of a nat.
2. Beq's notation is "==" instead of "=" now.
3. BNot's notation is "!" instead of "~" now, the notation level is also changed.
4. CIf's notation is "IF" instead of "TEST" now.
5. No Coq-globally-defined program variables any more. They are defined by new_var().
6. using the following notation:
Locate "If".
Locate "While".
Locate "Then".
Locate "Else".
...
Locate "EndWhile".
Locate "Skip".
7. subst defined syntactically

The following is a list of changes we would like to make in the future:
1. involving <, >, >=, !=
2. involving ||
3. involving unitary operator "-"
...
Axiom simpl_triple: forall P c Q R J,
  {{P}} c {{Q}} {{R}}{{J}} <-> {{assn_simpl P}} c {{assn_simpl Q}} {{assn_simpl R}}{{assn_simpl J}}.

Axiom elim_trivial_ex_derives: forall P Q P' Q',
  elim_trivial_ex P P' -> elim_trivial_ex Q  Q' -> (P |-- Q <-> P' |-- Q').

Axiom elim_trivial_ex_triple: forall P c Q P' Q' R R' J J',
  elim_trivial_ex P P' -> elim_trivial_ex Q  Q' -> 
  elim_trivial_ex R R' -> elim_trivial_ex J J' ->
  ({{P}} c {{Q}} {{R}} {{J}} <-> {{P'}} c {{Q'}} {{R'}} {{J'}} ).
