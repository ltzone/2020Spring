...
Proof.
  ...
  (** Here, [field_address tpair [StructField _a] p] means, [p] is a pointer
      ...
      the address of [_a] field. VST automatically knows [field_at] is the
      same as [data_at] [field_address]. *)
  (** Here, [forward_call] fails. It requires you to prove a side condition 
      first. *)
  assert_PROP (field_compatible tpair [StructField _a] p).
  { entailer. }
  forward_call (x,
                y,
    ...
    (* swap ( & (p -> a), & (p -> b) ); *)
  {
    ...
    f_equal; [| f_equal]; field_address_solver.
  }
